...
<div class="container mt-5">
    <p>
        This <span class="text-primary" style="cursor: pointer;" data-bs-toggle="modal" data-bs-target="#imageMessageModal">
            derived class
    ...
    <!-- Bootstrap Modal for ImageMessage -->
    <div class="modal fade" id="imageMessageModal" tabindex="-1" aria-labelledby="imageMessageModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="imageMessageModalLabel">Understanding Derived Classes in C#</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    A <b>derived class</b> in C# is a class that **inherits from a base class**, meaning it gains all the properties and methods of the base class but can also provide its own unique functionality. In this chat application, `ImageMessage` is a derived class of `MessageBase`, meaning it follows the structure defined in `MessageBase` but provides a custom implementation for `DisplayContent()` to support image messages.
